...
def Calculation(g, S_g):
    ...
    f = (
        ...
        )["FCAPf", "FCA", "Holder_act_y", "Holder_act_x"]
        .sum()
        .reset_index()
    )
...
def WeeklyCalculation(f):
    ...
    wfcapf = (
        f.groupby(["year_of_year", "week_of_year"])["FCAPf", "FCA"].mean().reset_index()
    )
...
for i in range(m):
    ...
    a = a[~(a.id < MaId)]
    # t.to_csv(
    #     path + "PartedNormalzedFCAP6.1" + "-Part" + str(i + 1) + ".csv", index=False
    # )
