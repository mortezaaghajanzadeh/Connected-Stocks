...
for i in range(1, 5):
    ...
    result = result.append(
        pd.read_pickle(path + "mergerd_first_step_weekly_all_part_{}.p".format(i)).drop(
            columns=[
                "Ret_x",
                "Ret_y",
                "SizeRatio",
...
Monthly = firstStep(result)
del result
#%%

# Monthly.reset_index(drop=True)[
#     ["t_Week", "Year_Month_week"]
# ].drop_duplicates().sort_values(by=["Year_Month_week"])
#%%
...
gg = Monthly.groupby(["t_Week"])

Monthly["WeeklyFCAP*"] = gg.WeeklyFCAPf.transform("rank")
gg = Monthly.groupby(["t_Week"])
tempt = gg["WeeklyFCAP*"].mean().to_frame()
mpingdict = dict(zip(tempt.index, tempt["WeeklyFCAP*"]))
Monthly["m"] = Monthly.t_Week.map(mpingdict)
Monthly["WeeklyFCAP*"] = Monthly["WeeklyFCAP*"] - Monthly.m
tempt = gg["WeeklyFCAP*"].std().to_frame()
mpingdict = dict(zip(tempt.index, tempt["WeeklyFCAP*"]))
Monthly["m"] = Monthly.t_Week.map(mpingdict)
Monthly["WeeklyFCAP*"] = Monthly["WeeklyFCAP*"] / Monthly.m
...
Monthly["WeeklyFCA*"] = gg.WeeklyFCA.transform("rank")
gg = Monthly.groupby(["t_Week"])
tempt = gg["WeeklyFCA*"].mean().to_frame()
mpingdict = dict(zip(tempt.index, tempt["WeeklyFCA*"]))
Monthly["m"] = Monthly.t_Week.map(mpingdict)
Monthly["WeeklyFCA*"] = Monthly["WeeklyFCA*"] - Monthly.m
tempt = gg["WeeklyFCA*"].std().to_frame()
mpingdict = dict(zip(tempt.index, tempt["WeeklyFCA*"]))
Monthly["m"] = Monthly.t_Week.map(mpingdict)
Monthly["WeeklyFCA*"] = Monthly["WeeklyFCA*"] / Monthly.m
...
print(len(Monthly))
Monthly = Monthly[~Monthly.Weeklyρ_5.isnull()]
print(len(Monthly),path)
n3 = path + "WeeklyAllPairsNormalzedFCAP9.1"
...
path = r"E:\RA_Aghajanzadeh\Data\Connected_Stocks\\"
n1 = path + "WeeklyAllPairsNormalzedFCAP9.1" + ".parquet"
...
#%%
print(len(df1))
df1 = df1.drop_duplicates(subset=["id", "t_Week"])
print(len(df1))
# %%
print(len(df1[(df1.WeeklyFCAPf >= 1)]))
df1 = df1[(df1.WeeklyFCAPf < 1)]
gg = df1.groupby(["t_Week"])
...
#%%
df1 = df1.sort_values(by=["id", "t_Week"])
...
# def quarter(g):
#     print(g.name)
#     q1 = g[g.WeeklyFCA > 0].WeeklyFCA.quantile(0.75)
#     mt = g[g.WeeklyFCA > 0].WeeklyFCA.quantile(0.5)
#     g.loc[g.WeeklyFCA > q1, "4rdQarter"] = 1
#     g.loc[g.WeeklyFCA > mt, "2rdQarter"] = 1
#     return g
...
def dailyn(g):
    ...
    a = g.groupby("uo").apply(Pnum).to_frame().rename(columns={0: "Positive"})
    a = a.merge(
        g.groupby("uo").apply(Nnum).to_frame().rename(columns={0: "Negative"}),
        how="left",
        on=["uo"],
    )
    # a["Grouped"] = 1
...
path = r"E:\RA_Aghajanzadeh\Data\Connected_Stocks\\"
n1 = path + "WeeklyAllPairsNormalzedFCAP9.2" + ".csv"
print(len(df1))
df1.to_csv(n1)
n1 = path + "WeeklyAllPairsNormalzedFCAP9.2" + ".parquet"
print(len(df1))
df1.to_parquet(n1)
