...
# %%
def target(df):
    wg = df.groupby(["symbol"])
    re = wg.apply(ResidualFactor)
    return re

def target2(df):
    return ResidualFactor(df)
    

# %%
symbols = PriceData["symbol"].unique()
symbols.sort()
ids = list(range(len(symbols)))
mapdict = dict(zip(symbols, ids))
PriceData["symbol_id"] = PriceData["symbol"].map(mapdict)

# %%
a1 = PriceData[PriceData.symbol_id < 2]
a2 = PriceData[(PriceData.symbol_id >= 3) & (PriceData.symbol_id < 4)]

# %%
from multiprocessing import Pool, cpu_count

p = Pool(cpu_count())
data = p.map(target2, a1[a1.symbol_id==0] )
data
...
p1 = multiprocessing.Process(target=target, args=a1)
p2 = multiprocessing.Process(target=target, args=a2)
...
)[["FCAPf", "FCA", "Holder_act_y", "Holder_act_x", "Holder_x"]].sum()
...
# %%
df[df.symbol == "فملی"].id.iloc[0], df[df.symbol == "فولاد"].id.iloc[0]


# %%
gdata = df.groupby(["id"])
g = gdata.get_group(376)
S_g = gdata.get_group(381)
