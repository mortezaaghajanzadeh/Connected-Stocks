...
for i in range(1,5):
    i = str(i)
    print(i)
    n1 = path + "NormalzedFCAP8.1-part" + i + ".parquet"
    d = pd.read_parquet(n1)
    d = firstStep(d,df)
...
assets = Monthly.groupby("id").size().to_frame()
assets = assets[assets[0] > 14].index
Monthly = Monthly[Monthly.id.isin(assets)].reset_index(drop=True)
n3 = path + "MonthlyNormalzedFCAP8.1"
...
n3 = path + "WeeklyNormalzedFCAP8.1"
