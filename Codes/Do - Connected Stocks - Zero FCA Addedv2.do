...
eststo v1: quietly asreg monthlyρ_5_f NMFCA sbgroup , fmb newey(4)
estadd loc GroupFE "No" , replace


eststo v2: quietly asreg monthlyρ_5_f NMFCA sbgroup  gdummy0-gdummy47, fmb newey(4)
estadd loc GroupFE "Yes" , replace


eststo v3: quietly asreg monthlyρ_5_f NMFCA sbgroup  monthlyρ_5   sgroup monthlysamesize monthlysamebm , fmb newey(4)
estadd loc GroupFE "No" , replace

eststo v4: quietly asreg monthlyρ_5_f NMFCA sbgroup monthlyρ_5   sgroup monthlysamesize monthlysamebm  gdummy0-gdummy47, fmb newey(4)
estadd loc GroupFE "Yes" , replace
...
asreg monthlyρ_5_f  median monthlyρ_5   sgroup monthlysamesize monthlysamebm   if MFCA != 0 , fmb newey(4)
estadd loc GroupFE "No" , replace

esttab    v1  v2 v3 v4 v5 v6 v7 , nomtitle label   s( N GroupFE r2 ,  lab("Observations" "Group FE" "$ R^2 $"))  keep(NMFCA   median monthlyρ_5 NMFCAM sbgroup  sgroup monthlysamesize monthlysamebm ) order(NMFCA sbgroup median NMFCAM )  compress  mgroups("Future Monthly Correlation of 4F+Industry Residuals"   , pattern(1 ) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span}) )  ,using ZeroBGSample.tex ,replace
