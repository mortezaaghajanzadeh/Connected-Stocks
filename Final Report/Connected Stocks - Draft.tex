...
For this purpose, we estimate cross-sectional regressions forecasting within-month realized correlation ($\rho_{i,j,t+1}$) of each stock pair's daily four-factor plus industry abnormal returns with $\text{FCA}^*_{ij,t}$, $\text{SameGroup}_{ij}$, and a host of pair characteristic that we use as controls:


\input{Model11}

We estimate these regressions monthly and report the time-series average as in \cite{FamaMacBeth} as a robust way to avoid any issues with cross-correlation in the residuals of equation \ref{model1}. We then calculate \cite{newey1987hypothesis} standard errors of the Fama-MacBeth estimates that tale into autocorrelation in the time series of cross-sectional estimates out to four lag ($ 4(60/100)^{\frac{2}{9}} = 3.57 \sim 4 $). Furthermore, we use the current period's co-movement to control other committed variables that depend on the pair's characteristics which we cannot measure. 
