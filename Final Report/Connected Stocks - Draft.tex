...
We extend our investigation in order to separate effect of business groups and high level of common ownership by constructing all the pairs in the market. For this purpose, we include stocks in one pair if they have, at least, two months in common. By this definition, we do not restrict our investigation to commonly hold stocks and set $\text{FCA}^*_{ij,t}$ to zero for two stocks that do not have any common owner. We define a dummy variable for that equals to one if pairs' common ownership, $\text{FCA}^*_{ij,t}$, is higher  than third quarter of that period and use it as our variable of interest. Other controls are as before:
\input{Model12}

We used the same methodology as used for estimating equation \ref{model1}. Table \ref{AllPairs} 
