...
%		\end{frame}   
	...
	\begin{frame}{Motivation}
		...
		\begin{itemize}
			\item\textbf{Does common ownership raise stock return co-movement?}
				
			\item \textbf{Would business groups be able to raise the co-movement of stock returns?}
			\item \textbf{What is the mechanism?}
%			\begin{itemize}
...
%			\end{itemize}
		\end{itemize}
					\begin{figure}
			\centering  
			\includegraphics[width=0.5\linewidth]{"Output/mcorr5l.eps"}
		\end{figure}
	\end{frame}
	...
	\begin{frame}{Related Literature}\label{Grapgh}
		\resizebox{\textwidth}{!}{
\input{Elements/Literature}
}
	\end{frame}
	
	\begin{frame}{Our work}
		\begin{itemize}
			\item We use daily records of block-holder ownership for firms (not restricted to mutual funds ownership)
		\item We connect stocks through the common ownership by blockholders (ownership $ > $ 1\%) for direct common ownership
		\item We connect stocks through the ultimate owner for indirect common ownership
...
\end{itemize}		
	...
	\section{Empirical Studies}
		...
		\begin{frame}{Pair composition}
			...
			\bigskip
			\pause
				\item  Pair in the Business Group:
								\pause
...
%														\pause
		...
		\begin{frame}{Pair Composition and Business Group}{Pair in the  Same Business Group }
			\centering
			\resizebox{0.5\textwidth}{!}{
		...
		\end{frame}
			...
			\begin{frame}{Pair Composition and Business Group}{Pair in the Separate Business Group }
   \centering
				\resizebox{0.5\textwidth}{!}{
					\centering
...
%				\pause
	...
	\input{Elements/No2BGShape.tex}
		...
		\begin{frame}{Number of Pairs}
			\begin{figure}[htbp]
				\includegraphics[width=0.95\linewidth]{Output/idMonth}
			\end{figure}
		\end{frame}
	%	
	...
	%		\end{frame}
		...
		\begin{frame}[t]{Measuring Common-ownership}\label{measure}
					\begin{columns}
					\scriptsize
				\column{.5\textwidth}    
				\centering  
	...
	%				\end{frame}
		...
		\begin{frame}{FCA's time series}
			\begin{figure}
				\centering  
				\includegraphics[width=0.9\linewidth]{"Output/FCAtimeSeries"}
	...
	%
		\begin{frame}{FCA's time series}
			\begin{columns}
				...
				\begin{figure}
					\centering  
					\includegraphics[width=\linewidth]{"Output/FCAtimeSeriesBG"}
					\pause
					\includegraphics[width=\linewidth]{"Output/FCAtimeSeriesPairType"}
										\pause
				\end{figure}    
				\column{.5\textwidth}
				\begin{figure}
					\centering  
					\includegraphics[width=\linewidth]{"Output/FCAComparetimeSeries"}
										\pause
					\includegraphics[width=\linewidth]{"Output/ComovementtimeSeries"}
				\end{figure}
	...
	%\end{frame}
					...
					\begin{frame}{MFCAP vs. FCAP Distributions}
						\label{Monthly1}
						\begin{columns}
							\column{.33\textwidth}  
							\begin{figure}
...
%	\end{frame}
	...
	\begin{frame}{Estimation model}\label{Estimation}
		\begin{itemize}
			...
			\item We use Fama-MacBeth method \scriptsize (As \cite{AntonPolk}) \normalsize
			\item Adjust standard errors by Newey and West adjustment with 4 lags ($ 4(70/100)^{\frac{2}{9}} = 3.69 \sim 4 $)  \scriptsize(As \cite{AntonPolk})
		\end{itemize}
				\hfill
...
\begin{frame}{Model Estimation}{High level of common ownership}
\label{Monthly9} 
...
\begin{frame}{Model Estimation}{All pairs}
	\begin{table}[htbp]
		\centering
		\resizebox{1\textwidth}{!}{
...
\end{frame}
 ...
 \subsection{Turnover}
	\begin{frame}{Turnover}{\cite{Liquidity2016}}
		\begin{equation*}
...
\begin{frame}{BG and correlation in Turnover}
	\begin{table}[htbp]
		\centering
		\resizebox{\textwidth}{!}{
			\centering
...
\begin{frame}{Abnormal Monthly Turnover}
\begin{itemize}

\item $ \text{Turnover}_{i,t} = \alpha_0 +\alpha_1 \times \text{Turnover}_{i,avg} +\alpha_2 \times \text{Turnover}_{m,t}  +\alpha_3 \times \text{Turnover}_{ind,t} + \boxed{\varepsilon_{i,t}} $
\begin{itemize}\scriptsize
...
\item $ \text{Turnover}_{m,t}$: Market turnover 
\item $ \text{Turnover}_{ind,t}$: Industry turnover 

\end{itemize}
\item Assign Abnormal Turnover to the business groups
\bigskip
...
\begin{frame}{Abnormal Monthly Turnover}{Standard error}
	\begin{table}[htbp]
		\centering
		\resizebox{0.8\textwidth}{!}{
...
\begin{frame}{Low abnormal monthly turnover standard error}
	\begin{table}[htbp]
\centering
		\resizebox{0.8\textwidth}{!}{
			\centering
...
\begin{frame}{Institutional Imbalance std}{Standard error}
%\begin{columns}
...
%\end{columns}
\end{frame}
\begin{frame}{Low Institutional Imbalance Group}
...
%			\item We derive a measure that captures the extent of common ownership distribution.
			...
			\item Business group affiliation is positively associated with higher stock
			return comovement.
			
			\item Among firm pairs that belong to the same business.
			groups, those with higher direct common ownership experience higher levels of
			return comovement.
			\item Simultaneous trades in the same direction among firms affiliated with the same business groups explains higher return comovements among those stocks. 
		\end{itemize}
	\end{frame}
