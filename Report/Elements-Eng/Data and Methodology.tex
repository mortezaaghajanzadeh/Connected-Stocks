...
\label{comovement}
	...
	Since we are interested in studying the impact of common ownership as well as business groups on stcok return comovement and that firms in the same business groups in Iran tend to be from the same industry, we would ideally want to subtract the impact of industry return in calculating abnormal returns. In addition, we know from prior literature that stocks from the same industry tend to comove together(cite?). Therefore, we use Fama-French four-factor model plus industry return to calculate abnormal returns, as shown below in equation (2). To measure the monthly stock return comovement for each firm pair at the end of each month, we first estimate our benchmark model using the past three months. Using the estimated coeffcients (betas), we then measure daily residuals and calculate the correlation of daily residuals from equation (2) during each month. 
	\begin{equation}
		\begin{split}
...
\subsection{Controls}
 ...
 Return comovement may be driven by stocks' intrinsinc similarities. We follow the literature (e.g. \cite{AntonPolk}) to control for potential drivers of stock return comovements which can be firm-specific as well as pair characteristitics. We separately control for size and book to market of both firms in a pair. Following \cite{AntonPolk}, we use the normalized rank-transform of the percentile market capitalization of the two stocks, \textbf{Size1} and \textbf{Size2}, where we label the
...
We also control for whether firms in a pair are similar in size and book to market: \textbf{SameSize}, and \textbf{SameBM} are the negative of the absolute difference in percentile ranking of size and book to market, respetively, of the two stocks in a pair.


In addtition, we control for whether the two stocks are in the same industry and business group, \textbf{SameIndustry}, \textbf{SameGroup}, respectively. (\textit{I believe we mention somewhere earlier that about ten percent of our pairs are in the same group?})About
\input{Elements/sameindandsamebg}of pairs are in the same industry and business group,  respectively. Furthermore, we control for cross-ownership between two stocks and define  \textbf{CrossOwnership} as the maximum percent of cross-ownership between the two firms in the following month\textit{ is there a particular reason for why this vairable is defined as forward looking?}.
...
We construct our control variables daily and take the monthly averages as the value of each variable at the end of each month. Panel \subref{ControlsSummary} table \ref{st2} reports the summary statistics of our control variables.
