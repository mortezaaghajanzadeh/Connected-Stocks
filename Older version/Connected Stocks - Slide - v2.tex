...
\AtBeginSection[]
{
	\begin{frame}{Table of Contents}
		\tableofcontents[hideallsubsections,currentsection,hideothersubsections]
	\end{frame}
}	
...
\end{frame}
	...
	\begin{figure}
		\centering  
		\includegraphics[width=0.9\linewidth]{"BGtimeSeries.eps"}
...
%	\end{frame}
	...
	\subsection{Sum Factor}
	
	
	\begin{frame}{ Future Correlation via $ FCAP* $}{Normalized Rank Transformed }\label{Monthly18}
	...
	\begin{frame}{Fama-MacBeth Estimation}{Normalized Rank Transformed }
		\label{Monthly17} 
		\begin{table}[htbp]
			\centering
			\resizebox{1\textheight}{!}{
...
\begin{frame}{Business group \& Common-ownership}{regression}
	\begin{table}[htbp]
		...
		\resizebox{0.6\textwidth}{!}{
			\input{subsampleBGmresult-slide.tex}
		}
	\end{table}
\end{frame}
...
%	\begin{frame}{Correlation summary}
%				\begin{table}[htbp]
%			\centering 
%			\scriptsize
%			\resizebox{\textwidth}{!}{
%				\input{6CorrelationsResults.tex}              
%			}
%		\end{table}
%	\end{frame}
%
%\begin{frame}{4Factor + Industry + Bgroup}
%	\begin{table}[htbp]
%	\centering
...
%			\item Use Fama-MacBeth to estimate this model
%			\input{BulishModel}
%			\item Bearish/Bullish definition :
%			\begin{itemize}
%				\item $ \text{Bullish Market} : R_{TSE,Monthly} \geq 2\%  $ : 34 Month
%				\item $ \text{Bearish Market} : R_{TSE,Monthly} \leq -2\%  $ : 11 Month
%			\end{itemize}
%		\end{itemize}
%	\end{frame}
%	
%	
%	\begin{frame}{Model Estimation}{Normalized Rank-Transformed (Bearish Market)}\label{Monthly20}
%		
%		
%		\begin{table}[htbp]
%			\centering
%			\resizebox{0.7\textwidth}{!}{
%				\input{mresult2Down-slide2.tex}
%			}
%		\end{table}
%		
%		
...
%	
%	
%	\subsection{Effective Business Group}
	
	%\begin{frame}[t]{Business Group Dummy}
	%\begin{itemize}
	%\item We use group dummies and its interaction with $ FCA^* $
	%
	%\begin{itemize}
	%
	%%\item Model with base:
	%%\begin{equation*}
	%%\scriptsize
	%% \begin{split}
	%% \rho_{ij,t+1} = & \text{ 	}\beta_0   + \beta_1* \text{FCA}_{ij,t}^*  + \beta_2 * \text{SameGroup}_{ij} 	+\beta_3 * FCA_{ij,t}^* *\ \text{SameGroup}_{ij}\\
	%% &   + \sum_1^G \lambda_{1,g} * \delta_{ij,g} + \sum_1^G \lambda_{2,g} * \delta_{ij,g}  * \text{FCA}_{ij,t}^* \\
	%% & + \sum_1^G \lambda_{3,g} *\gamma_{g}  + \sum_{k=1} ^{n} \alpha_k*\text{Control}_{ij,t} + \varepsilon_{ij,t+1}
	%% \end{split}
	%%\end{equation*}
	%%
	%%\item Model without base:
	% \item[]
	%\input{GroupModel.tex}
	% 
	%
	%
	%
	...
	%\begin{itemize}
	%\scriptsize
	%\item $ \delta_{ij,g} = \text{SameGroup}_{ij} * \gamma_g $ which $ \gamma_g $ is a business group dummy
	%\end{itemize}
	%
	%\end{itemize}
	%\end{frame}
	...
	%
	%
	%\begin{frame}{Effective Business Group}{Significant coefficients}
	%
	%\input{GroupsRe}
	%
	%
...
%	
%	
%	\begin{frame}{Effective Business Group}{Check banking }
%		\begin{itemize}
%			\item We define bank in group
%			\begin{itemize}
%				\footnotesize
%				\item \textbf{Bank In Group:} Groups that ,at least, consist of one bank as a member or ultimate owner
%				%\item \textbf{Inv. In Group:} Groups that ,at least, consist of one investment firm
%			\end{itemize}
...
%	\end{frame}
%	
%	\begin{frame}{Effective Business Group}{Check banking business groups}
%		\begin{table}[htbp]
%			\centering
%			\resizebox{0.7\textwidth}{!}{
%				\input{mresult2BankIn.tex}
%			}
%		\end{table}
%	\end{frame}
